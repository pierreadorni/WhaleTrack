...
def pixel_to_gps(
    ...
    #lat_obj = drone_lat + np.degrees(dlat)
    #lon_obj = drone_lon + np.degrees(dlon)

    return np.degrees(lat_ob_rad), np.degrees(lon_obj_rad)
...
if __name__ == "__main__":
    ...
    VIDEO_ACQUISITION = "DJI_20260116155837_0003_V"
    flight_info_dict = get_sub_info_dict(
        data_json_dict=DATA_JSON, key="flight_day", value=FLIGHT_DAY
    )
    ...
    for index, row in acquisition_data_csv.iterrows():
        ...
        if image is not None:
            ...
            _, _ = save_frame_with_fixed_point(
                image=image,
